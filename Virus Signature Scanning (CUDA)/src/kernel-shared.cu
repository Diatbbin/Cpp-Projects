...
__global__ void matchFile(const uint8_t* file_data, size_t file_len, char** signatures, size_t* lens, int* founds)
{
    ...
    // Store signature in shared memory
    extern __shared__ char signature[];
    for (int i = threadIdx.x; i < lens[blockIdx.x]; i += blockDim.x)
    {
        signature[i] = signatures[blockIdx.x][i];
    }
...
void runScanner(std::vector<Signature>& signatures, std::vector<InputFile>& inputs)
{
    ...
    for(size_t i = 0; i < inputs.size(); i++)
    {
        ...
        /*
        size* ptr = 0;
        check_cuda_error(cudaMallocManaged(&ptr, sizeof(size_t)));
        cudaMemcpy(ptr, inputs[i].size, sizeof(size_t), cudaMemcpyHostToDevice);
        file_size_bufs.push_back(ptr); */
    }
...
/*
        ...
        {   */
             ...
             */
        matchFile<<<signatures.size(), 512, 8192 * sizeof(char), streams[file_idx]>>>(
