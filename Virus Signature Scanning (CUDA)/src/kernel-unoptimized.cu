...
__global__ void matchFile(const uint8_t* file_data, size_t file_len, const char* signature, size_t len, int* found)
{
    // TODO: your code!
...
void runScanner(std::vector<Signature>& signatures, std::vector<InputFile>& inputs)
{
    ...
    for(size_t file_idx = 0; file_idx < inputs.size(); file_idx++)
    {
        ...
        {   
             ...
             */
            matchFile<<<1, 512, /* shared memory per block: */ 0, streams[file_idx]>>>(
                    file_bufs[file_idx], inputs[file_idx].size,
                    sig_bufs[sig_idx], signatures[sig_idx].size, &results[file_idx][sig_idx]);
